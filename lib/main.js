...
function abrePopup()
{
    ...
    else
    {
        ...
        {
            if (tab.url == "http://noteboardapp.com/board.php")
            {
                tabOpen = true;
                tab.activate();
            }
        }
        if (!tabOpen) tabs.open("http://noteboardapp.com/board.php");
    }
}
...
function abreLogin()
{
    ...
    panel.port.on("loged", function(datos){
        ...
        panel.hide();
        if (primera) tabs.open("http://noteboardapp.com/loginPageFire.php");
    ...
    panel.port.on("registered", function(datos){
        ...
        panel.hide();
         if (primera) tabs.open("http://noteboardapp.com/loginPageFireReg.php");
        listenFacebook = false;
    });
    panel.port.on("goDemo", function(datos){
        ...
        panel.hide();
        if (primera) tabs.open("http://noteboardapp.com/board.php?user=demo");
        listenFacebook = false;
    });
}
    ...
    if (tab.url == 'http://noteboardapp.com/loginPageFire.php')
    {
            worker = tab.attach({
                contentScriptFile:  [data.url("jquery/jquery.js"),
                                     data.url("varios.js"),
    ...
    if (tab.url == 'http://noteboardapp.com/loginPageFireReg.php')
    {
            worker = tab.attach({
                contentScriptFile:  [data.url("jquery/jquery.js"),
                                     data.url("varios.js"),
    ...
    if (tab.url == 'http://noteboardapp.com/loginPage.php')
    {
    ...
    if (listenFacebook && tab.url.indexOf('www.facebook.com/connect/login_success.html') > -1)
    {
        ...
        if (tab.url.indexOf('#access_token') > -1)
            ...
            // else
            // {
            //     sendMessageToInjectCode(loginTab, {answerLogin: "Error"});
            // }
    ...
    onComplete: function(answer){
        ...
        // {
            ...
            if (resp.isSuccess)
            {
                ...
                panel.hide();
                if (resp.nuevoReg) tabs.open("http://noteboardapp.com/loginPageFireReg.php");
                else tabs.open("http://noteboardapp.com/loginPageFire.php");
                // if (resp.nuevoReg) tabs.open("http://noteboardapp.com/paypalPage3.php");
                // else tabs.open("http://noteboardapp.com/board.php");
            }
        // }
    }
  }).post();
...
function crearMenus()
{
    ...
    if (ss.storage.listaTablones)
    {
       ...
       for (var i=0;i< ss.storage.listaTablones.length-1;i++)
        {
            var numtab = i + 1;
            numtab += '';
            menuImg2 =  menu.Item({
...
function guardaPostit(type,url,data,numTablon){
    ...
    //var numTablon = buscarMenu(menuItemId);
    // var numTablon = 1;
    var f = new Date();
