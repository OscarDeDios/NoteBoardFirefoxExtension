...
if (!ss.storage.installed && !ss.storage.password)
{
    tabs.open("https://www.noteboardapp.com/firefoxExtensionWelcome");
    ss.storage.installed = true;
}
...
function abrePopup()
{
    ...
    else
    {
        ...
        {
            if (tab.url == "https://www.noteboardapp.com/firefoxBoard" ||
                tab.url == "https://www.noteboardapp.com/board" ||
                tab.url == "https://www.noteboardapp.com/board.php")
            {
                tabOpen = true;
                tab.activate();
            }
        }
        if (!tabOpen) tabs.open("https://www.noteboardapp.com/firefoxBoard");
    }
}
...
function abreLogin()
{
    ...
    panel.port.on("loged", function(datos){
        ...
        panel.hide();
        if (primera) tabs.open("https://www.noteboardapp.com/loginPageFire");
    ...
    panel.port.on("registered", function(datos){
        ...
        panel.hide();
         if (primera) tabs.open("https://www.noteboardapp.com/loginPageFireReg");
        listenFacebook = false;
    });
    panel.port.on("goDemo", function(datos){
        ...
        panel.hide();
        if (primera) tabs.open("https://www.noteboardapp.com/boarddemo");
        listenFacebook = false;
    });
        ...
        tab.url == "https://www.noteboardapp.com/board.php")
    {
        ...
        worker.port.on('creaNotificacion', function(param){
            var notifications = require("sdk/notifications");
            notifications.notify({
              title: "Note Board",
              text: param.message,
              iconURL: data.url("imagenes/NoteBoard.png"),
              onClick: function (data) {
                sendMessageToInjectCode({notificationClicked: param.numPost});
    ...
    if (tab.url == 'https://www.noteboardapp.com/loginPageFire')
    {
            worker = tab.attach({
                contentScriptFile:  [data.url("jquery/jquery.js"),
                                     data.url("varios.js"),
    ...
    if (tab.url == 'https://www.noteboardapp.com/loginPageFireReg')
    {
            worker = tab.attach({
                contentScriptFile:  [data.url("jquery/jquery.js"),
                                     data.url("varios.js"),
    ...
    if (tab.url.indexOf('noteboardapp.com') > -1 && tab.url.indexOf('firefoxBoard') == -1 && tab.url.indexOf('/board') == -1)
    {
            worker = tab.attach({
    ...
    if (tab.url == 'https://www.noteboardapp.com/loginPage')
    {
...
{
  var url = 'https://www.noteboardapp.com/api/loginFb.php';
  ...
  Request({
    ...
    onComplete: function(answer){
        ...
        if (resp.isSuccess)
        {
            ...
            panel.hide();
            if (resp.nuevoReg) tabs.open("https://www.noteboardapp.com/loginPageFireReg");
            else tabs.open("https://www.noteboardapp.com/loginPageFire");
            // if (resp.nuevoReg) tabs.open("https://www.noteboardapp.com/paypalPage3.php");
            // else tabs.open("https://www.noteboardapp.com/board.php");
        }
    }
  }).post();
}
...
function guardaPostit(type,url,data,numTablon){
    ...
    Request({
      url: 'https://www.noteboardapp.com/api/createNoteFirefox.php',
      content: param,
      onComplete: function(answer){
             var resp = answer.json;
             if (resp.isSuccess)
